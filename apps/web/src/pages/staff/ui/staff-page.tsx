...
function getSelectedRoles(formData: FormData): StaffAssignableRole[] {
  const roles = formData
    .getAll("roles")
    .filter((role): role is StaffAssignableRole => role === "teacher" || role === "principal");

  return roles.length > 0 ? roles : ["teacher"];
}

function getStaffFormInput(formData: FormData): Omit<StaffMemberCreateInput, "roles"> & {
  roles: StaffAssignableRole[];
} {
  return {
    ...
    joinedOn: getNullableString(formData, "joinedOn"),
    leftOn: getNullableString(formData, "leftOn"),
...
function getAccessStatusLabel(staff: StaffMember) {
  switch (staff.accessStatus) {
    ...
    case "not_invited":
      return m.staff_page__access_not_invited();
  }
}
...
function getRoleLabel(role: StaffAssignableRole) {
...
function RoleControls({
  ...
  defaultRoles: readonly StaffAssignableRole[];
  disabled?: boolean;
}) {
  return (
    <Field>
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        <label className="flex items-center gap-2 rounded-md border p-3 text-sm">
          <input
            aria-label={m.staff_page__role_teacher()}
            className="h-4 w-4 rounded border-input"
            defaultChecked={defaultRoles.includes("teacher")}
            disabled={disabled}
            name="roles"
            type="checkbox"
            value="teacher"
        ...
        <label className="flex items-center gap-2 rounded-md border p-3 text-sm">
          <input
            aria-label={m.staff_page__role_principal()}
            className="h-4 w-4 rounded border-input"
            defaultChecked={defaultRoles.includes("principal")}
            disabled={Boolean(disabled) || !canManagePrincipalRole}
            name="roles"
            type="checkbox"
            value="principal"
          />
          ...
          <FieldGroup className="gap-4">
            <div className="grid gap-4 sm:grid-cols-2">
              <Field>
                <FieldLabel htmlFor="staff-create-full-name">
                  {m.staff_page__full_name()}
                </FieldLabel>
                <Input id="staff-create-full-name" name="fullName" required />
              </Field>
              <Field>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field>
                ...
                <Input id="staff-create-employee-code" name="employeeCode" required />
              </Field>
              <Field>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Field>
                <FieldLabel htmlFor="staff-create-phone">{m.staff_page__phone()}</FieldLabel>
                <Input id="staff-create-phone" name="phone" />
              </Field>
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
              <Field>
                <FieldLabel htmlFor="staff-create-joined-on">
                  {m.staff_page__joined_on()}
                </FieldLabel>
                <Input id="staff-create-joined-on" name="joinedOn" type="date" />
              </Field>
              <Field>
...
function StaffMemberCard({
  ...
  const canManageThisStaff = canManageStaff && (!isPrincipal || canManagePrincipalRole);
  ...
  const handleUpdateSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const input: StaffMemberUpdateInput = {
      ...getStaffFormInput(formData),
      id: staff.id
    };
  ...
  const handleAccess = async () => {
    ...
    try {
      ...
      if (staff.accessStatus === "linked") {
        await revokeAccessMutation.mutateAsync({ staffProfileId: staff.id });
        toast.success(m.staff_page__access_revoked_success());
      } else {
        await grantAccessMutation.mutateAsync({ staffProfileId: staff.id });
      ...
      <div className="flex flex-col gap-3 border-b px-4 py-3 md:flex-row md:items-start md:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <h3 className="truncate text-sm font-medium">{staff.fullName}</h3>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {staff.employeeCode} · {staff.roles.map(getRoleLabel).join(", ")}
          </p>
        </div>
        {canManageStaff ? (
          <StaffAccessButton
        ...
        <FieldSet disabled={!canManageThisStaff}>
          <FieldGroup className="gap-4">
            ...
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <Field>
                <FieldLabel htmlFor={`staff-${staff.id}-phone`}>{m.staff_page__phone()}</FieldLabel>
                <Input
                  defaultValue={staff.phone ?? ""}
                  id={`staff-${staff.id}-phone`}
                  name="phone"
                />
              </Field>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-3">
              <Field>
                <FieldLabel htmlFor={`staff-${staff.id}-joined-on`}>
                  {m.staff_page__joined_on()}
                </FieldLabel>
                <Input
                  defaultValue={staff.joinedOn ?? ""}
                  id={`staff-${staff.id}-joined-on`}
                  name="joinedOn"
                  type="date"
                />
              </Field>
              <Field>
                <FieldLabel htmlFor={`staff-${staff.id}-left-on`}>
                  {m.staff_page__left_on()}
                </FieldLabel>
                <Input
                  defaultValue={staff.leftOn ?? ""}
                  id={`staff-${staff.id}-left-on`}
                  name="leftOn"
                  type="date"
                />
              </Field>
